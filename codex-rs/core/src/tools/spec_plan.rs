...
fn add_collaboration_tools(context: &CoreToolPlanContext<'_>, registry: &mut ToolRegistry) {
    ...
    if collab_tools_enabled(turn_context, context.model_info) {
        if multi_agent_v2_enabled(turn_context) {
            ...
            // External providers require a plaintext route outside the encrypted namespace.
                ...
                std::iter::once((tool_namespace, encrypt_messages)).chain(external_route)
            {
                let catalog_messages =
                    (tool_namespace != Some(EXTERNAL_AGENTS_NAMESPACE)).then_some(&model_messages);
                registry.register_trusted_with_exposure(
                    multi_agent_v2_handler(
                        SpawnAgentHandlerV2::new(
                            ...
                            (tool_namespace != Some(EXTERNAL_AGENTS_NAMESPACE))
                                .then_some(spawn_agent_description)
                                .flatten()
                                .map(str::to_owned),
                        ),
                        tool_namespace,
                        ...
                        None,
                        catalog_messages.and_then(|messages| {
                            messages.multi_agent_tool_parameters_override("spawn_agent")
                        }),
                    ),
                    exposure,
                );
                if !turn_context.config.multi_agent_v2.disable_direct_message {
                    registry.register_trusted_with_exposure(
                        multi_agent_v2_handler(
                            ...
                            encrypt_messages,
                            catalog_messages.and_then(|messages| {
                                messages.multi_agent_tool_description_override("send_message")
                            }),
                            catalog_messages.and_then(|messages| {
                                messages.multi_agent_tool_parameters_override("send_message")
                            }),
                        ),
                        exposure,
                    );
                    registry.register_trusted_with_exposure(
                        multi_agent_v2_handler(
                            ...
                            encrypt_messages,
                            catalog_messages.and_then(|messages| {
                                messages.multi_agent_tool_description_override("followup_task")
                            }),
                            catalog_messages.and_then(|messages| {
                                messages.multi_agent_tool_parameters_override("followup_task")
                            }),
                        ),
                        exposure,
                    );
                ...
                if turn_context.config.multi_agent_v2.wait_agent_enabled {
                    registry.register_trusted_with_exposure(
                        multi_agent_v2_handler(
                            ...
                            encrypt_messages,
                            catalog_messages.and_then(|messages| {
                                messages.multi_agent_tool_description_override("wait_agent")
                            }),
                            catalog_messages.and_then(|messages| {
                                messages.multi_agent_tool_parameters_override("wait_agent")
                            }),
                        ),
                        exposure,
                    );
                ...
                registry.register_trusted_with_exposure(
                    multi_agent_v2_handler(
                        ...
                        encrypt_messages,
                        catalog_messages.and_then(|messages| {
                            messages.multi_agent_tool_description_override("interrupt_agent")
                        }),
                        catalog_messages.and_then(|messages| {
                            messages.multi_agent_tool_parameters_override("interrupt_agent")
                        }),
                    ),
                    exposure,
                );
                registry.register_trusted_with_exposure(
                    multi_agent_v2_handler(
                        ...
                        encrypt_messages,
                        catalog_messages.and_then(|messages| {
                            messages.multi_agent_tool_description_override("list_agents")
                        }),
                        catalog_messages.and_then(|messages| {
                            messages.multi_agent_tool_parameters_override("list_agents")
                        }),
                    ),
                    exposure,
                );
